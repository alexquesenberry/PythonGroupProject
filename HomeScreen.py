...
class HomeScreen(object):
    ...
    def Button_Scores(self):
        ...
        click_sound.play()
        scorescreen = ScoresScreen(screen, WIDTH, HEIGHT)
        scorescreen.run()
    ...
    def Add_Buttons(self):
        ...
        if CENTER_SCREEN - 75 < mouse[0] < CENTER_SCREEN + 75 and 300 < mouse[1] < 350:
            pygame.draw.rect(screen, BUTTON_HOVER,
                             (CENTER_SCREEN - 75, 300, 150, 50))
            if release[0] == 1:
                self.Button_Play()
        else:
...
if __name__ == "__main__":
    ...
    main_background = pygame.image.load(
        "res/background_main.jpg").convert()  # load background image
